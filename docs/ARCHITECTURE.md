...
- `ConfigDB`：启动时读取场景、升级、帮手、天赋和防守关卡配置。
- `GameState`：保存资源、当前场景、升级等级、天赋、已解锁内容和各场景待收集产物库存。
...
- `TalentSystem`：校验天赋点与前置节点并点亮永久天赋。

## UI

- `HUD`：顶部现金、回收物数量和场景切换入口。
- `BottleSpawnArea`：更新全部已解锁场景的独立生产计时器，并在渲染预算内把当前场景库存显示为可点击目标；千级库存不会全部实例化为节点。
- `UpgradePanel`：底部升级、场景、帮手和天赋页签，以及升级子页签。
- `TalentTree`：显示可双向拖动的四向天赋网，处理节点点击和状态反馈。
